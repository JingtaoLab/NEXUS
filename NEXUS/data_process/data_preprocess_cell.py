...
class Data_process:
    def __init__(self, input_file_path, num_of_highly_variable_genes, split_ratio, batch_size, device_use, process=True):
        ...
        self.use_ddp = False  # 标记是否使用DDP
        self.world_size = 1  # 进程数量

    def read_all_adata(self):
        # 读取combined_adata
        if self.input_file_path.endswith(".h5ad"):
            # 读取h5ad文件（如果是整合好的文件，直接读取h5ad,需要是原始counts）
            ...
            # 读取h5ad文件（如果不是整合好的文件，从文件夹中读取并整合，需要是原始counts）
            scRNA_datas = []
        ...
        if self.process:
            self.combined_adata = self.combined_adata[self.combined_adata.X.sum(axis=1) > 0]    # 过滤空细胞
    ...
    def setup_DDP(self):
        if self.device_use == "gpu":
            ...
            # 检查环境变量是否已设置（可能由外部启动脚本如torchrun设置）
            rank = os.environ.get('RANK', None)
            world_size = os.environ.get('WORLD_SIZE', None)
            
            if rank is not None and world_size is not None:
                # 环境变量已设置，使用多进程启动（如torchrun）
                rank = int(rank)
                ...
                # 多个GPU但环境变量未设置，可以选择自动设置环境变量使用多GPU DDP
                # 或者使用单GPU模式
                # 这里我们自动设置环境变量以使用所有可用GPU
                ...
                # 自动设置环境变量以使用所有GPU（单进程多GPU模式）
                os.environ['RANK'] = '0'
                os.environ['WORLD_SIZE'] = str(num_gpus)
                os.environ['MASTER_ADDR'] = 'localhost'
                os.environ['MASTER_PORT'] = '12355'
                
                # 注意：单进程多GPU DDP需要特殊处理，这里我们使用单GPU模式
                # 真正的多GPU DDP应该使用torchrun启动
            ...
            self.use_ddp = True  # CPU模式下也使用DDP结构
            self.world_size = 1
